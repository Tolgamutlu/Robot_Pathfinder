...
namespace RobotNav
{
    public class ASL
    {
        ...
        //constructors
        public ASL(string fn, Agent a, Env e, List<Wall> w, Goals g, int dl)
        {
            fileName = fn;
        ...
        public string GetBestNode()
        {
            ...
            {
                int number_of_nodes = node.Length + GetHeuristic(node);                                    //gives each node in frontier a number_of_nodes using Heuristic
                if (number_of_nodes < minCost)                                                             //The node with the lowest code is stored as BestNode
                {
                    minCost = number_of_nodes;
                    bestNode = node;
                }
            }
